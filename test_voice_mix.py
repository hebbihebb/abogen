#!/usr/bin/env python3
"""Create a custom voice mix with Kokoro, then clone it with F5-TTS."""
import sys
...
def create_mixed_voice_reference():
    """Create a custom voice mix using Kokoro's voice blending."""
    print("=" * 70)
    ...
    # Voice mix formula as string
    # Format: "voice1*weight1 + voice2*weight2 + ..."
    voice_formula = "af_bella*0.6 + af_nicole*0.5 + af_sarah*0.4 + am_adam*0.3"

    print("\nVoice Mix Formula:")
    print(f"  {voice_formula}")
    ...
    print(f"\nGenerating mixed voice...")
    print(f"Text: {ref_text[:80]}...")

    # Synthesize with voice mixing
    audio_chunks = []
    sample_rate = None

    for result in engine(ref_text, voice=voice_formula):
