#!/usr/bin/env python3
"""Create a custom voice mix by manually blending Kokoro voice embeddings."""
import sys
...
def create_mixed_voice_reference():
    """Create a custom voice mix by manually loading and blending voices."""
    print("=" * 70)
    ...
    # Load individual voice embeddings and blend them
    print("\nLoading and blending voice embeddings...")
    mixed_voice = None
    total_weight = sum(w for _, w in voice_weights)

    for voice_name, weight in voice_weights:
        print(f"  Loading {voice_name}...")
        voice_embedding = engine.load_single_voice(voice_name)

        if mixed_voice is None:
            mixed_voice = voice_embedding * weight
        else:
            mixed_voice = mixed_voice + (voice_embedding * weight)

    # Normalize by total weight
    mixed_voice = mixed_voice / total_weight

    print(f"[OK] Created blended voice embedding")
    ...
    print(f"\nGenerating audio with mixed voice...")
    print(f"Text: {ref_text[:80]}...")

    # Synthesize with the mixed voice embedding directly
    audio_chunks = []
    sample_rate = None

    # Use the pipeline with the pre-mixed voice embedding
    for result in engine.pipeline(
        ref_text,
        voice=mixed_voice,  # Pass the mixed embedding directly
        speed=1.0,
    ):
...
if __name__ == "__main__":
    print("\nKokoro Voice Mix + F5-TTS Cloning Test (Manual Mixing)\n")
